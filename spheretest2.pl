...
for (my $b = 20 ; $b-- ;){
$rside[$_] = Cuboid->new();
$rside[$_]->setDimensions(1,$sy,$sz);
$tdc->registerObject($rside[$_],\@focuspoint,'#9999FF',$screenx,$sy*$a,$sz*$b);
}
}
for (my $a = 19 ; $a>9 ;$a--){
for (my $b = 20 ; $b-- ;){
...
for (my $b = 20 ; $b-- ;){
$lside[$_] = Cuboid->new();
$lside[$_]->setDimensions(1,$sy,$sz);
$tdc->registerObject($lside[$_],\@focuspoint,'#9999FF',0,$sy*$a,$sz*$b);
}
}
for (my $a = 19 ; $a>9 ;$a--){
for (my $b = 20 ; $b-- ;){
...
{
my $index=shift;
#may move to 3D library if it works well enough - not general purpose enough at the moment
...
sub _shadowMK2
{
	...
	#only dealing with a sphere so fairly easy
	#get 8 points on circle inside sphere in plane perpendicular to light source,
	#then trace vector lightsource to point through to where it hits the wall
	#not general purpose enough at the moment
	my $centre = $spheres[$index][0]->getCentre();
	my @lightvector = ($$centre[0] - $lightsource[0],$$centre[1]-$lightsource[1],$$centre[2]-$lightsource[2]);
	_normalise(\@lightvector);
	...
	@{$spheres[$index][5]->{VERTEXLIST}}=@{$tempobj->{VERTEXLIST}};
}
